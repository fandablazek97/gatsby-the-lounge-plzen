...
export default function LanguageSwitcherMobile() {
  const [selected, setSelected] = useState(langs[2]);
        ...
        <Listbox.Options className="focus-visible:a11y-white absolute left-0 top-8 flex w-36 flex-col items-center justify-between space-y-5 rounded-md bg-body bg-opacity-80 p-5 !outline-none backdrop-blur-lg">
          {langs.map((lang) => (
            <Listbox.Option
              key={lang.id}
              value={lang.label}
              className="cursor-!outline-none focus-visible:a11y-white text-xs font-semibold uppercase tracking-wider text-rich transition-opacity duration-150 hover:opacity-70  xs:text-sm"
            >
